...
namespace SimplyRugby.MatchAndTrainingAmateur.MatchLogAmateur
{
    /// <summary>
    /// Page for logging match details for amateur teams.
    /// </summary>
    public sealed partial class MatchLogAmateur : Page
    {
        public MatchLogAmateur()
        {
        ...
        private async void SubmitScores_Click(object sender, RoutedEventArgs e)
        {
            ...
            // Further validations can be added here, such as checking if scores are within a certain range if necessary

            if (dateOfMatch > DateTime.Today)
            {
                ShowErrorMessage("The date of the match cannot be in the future.");
                return;
            }
                ...
                try
                {
                    using (var connection = new MySqlConnection(connectionString))
                    {
                        ...
                        using (var command = new MySqlCommand(insertQuery, connection))
                        {
                            ...
                            {
                                ShowSuccessMessage();
                                Frame.Navigate(typeof(MatchAndTrainingAmateur)); // Refresh the page
                            }
                            else
                            {
